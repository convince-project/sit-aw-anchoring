...
public:
  ...
  std::vector<std::string> generatePopulateInstanceQueries(const json& elem) override
  {
    ...
    //
    // #1, place affordance describing placeability, support and can_place dispositions
    q =
      ...
      "insert $pa1 (defines: $p, describes: $cp, describes: $pd, describes: $sd) isa place_affordance; "
             ...
             // preconditions
             "$req_in_front isa required_is_in_front_of; "
             "$req_clear    isa required_is_clear; "
             "(affordance: $pa1, required: $req_in_front) isa affordance_precondition; "
             "(affordance: $pa1, required: $req_clear)    isa affordance_precondition; ";
    queries.push_back(q);

    // #2, place affordance describing placeability, storage and can_place dispositions
    q =
      "match $p  isa place, has id \""   + id     + "\"; "
            // concerned dispositions
            "$pd isa placeability; "
            "$gd isa storage; "
            "$cp isa can_place; "
      "insert $pa2 (defines: $p, describes: $cp, describes: $pd, describes: $gd) isa place_affordance; "
             ...
             "$req_in_front isa required_is_in_front_of; "
             "$req_clear    isa required_is_clear; "
             "(affordance: $pa2, required: $req_in_front) isa affordance_precondition; "
             "(affordance: $pa2, required: $req_clear)    isa affordance_precondition; ";
    queries.push_back(q);
  ...
  virtual std::vector<std::string> generateUpdateStateQueries(const std::string& inst_id, const json& dt_data) override
  {
    ...
    // if last place execution had no success
    if (attr_values["exec_result"].compare("success") != 0) {
      // Update the player roles of the role assignments to point to wo, wh and wr (3a)
      q.append(
      "insert (assigned_role: $wor, player: $wo, context: $s) isa role_assignment; "
           ...
           "(assigned_role: $wrr, player: $wr, context: $s) isa role_assignment;"  // (3a) done
      );
    }
    else {
    // last place execution had success
      // Update the role assignments to point to wo, wh and wr, without any player roles (3b)
      q.append(
      "insert (assigned_role: $wor, context: $s) isa role_assignment; "
           "(assigned_role: $wtr, context: $s) isa role_assignment; "
           "(assigned_role: $wrr, context: $s) isa role_assignment;"  // (3b) done
      );
    }
